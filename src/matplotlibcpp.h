...
namespace matplotlibcpp
{
    namespace detail
    {
        ...
        struct _interpreter
        {
            ...
            PyObject *s_python_function_invert_yaxis;
        ...
        private:
            ...
            _interpreter()
            {
                ...
                s_python_function_invert_yaxis = PyObject_GetAttrString(pymod, "invert_yaxis");
...
// a non-numpy alternative for `detail::get_2darray()`.
    ...
    inline void invert_yaxis()
    {
        ...
        if (!ax)
            throw std::runtime_error("Call to invert_yaxis() failed.");
        Py_INCREF(ax);
        PyObject *invert_yaxis = PyObject_GetAttrString(ax, "invert_yaxis");
        if (!invert_yaxis)
            throw std::runtime_error("Attribute set_aspect not found.");
        Py_INCREF(invert_yaxis);
        PyObject *args = PyTuple_New(0);
        PyObject *kwargs = PyDict_New();
        PyObject *res = PyObject_Call(invert_yaxis, args, kwargs);
        if (!res)
            throw std::runtime_error("Call to set_aspect() failed.");
        Py_DECREF(invert_yaxis);
        Py_DECREF(args);
        Py_DECREF(kwargs);
        Py_DECREF(ax);
    }
